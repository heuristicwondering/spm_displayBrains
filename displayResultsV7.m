function [ ] = displayResultsV7(varargin)
...
mfDRListfig = figure( 'Name', 'Saved Brains', 'Position', [50 300 1500 700], 'ToolBar', 'none', 'MenuBar', 'none' );
...
function [] = saveBrainsCallback(source, event)
    global mfDRListCon mfDRSaveList;
    fileName = uiputfile;
    if fileName
        lbstrings = get( mfDRListCon, 'String' );
        save( fileName, 'mfDRSaveList', 'lbstrings' );
    end
end

function [] = openBrainsCallback(source, event)
    global mfDRListCon mfDRSaveList;
    fileName = uigetfile;
    if fileName
        load( fileName, 'mfDRSaveList', 'lbstrings' );
...
function [] = keydownCallback(~, event)
    global mfDRListfig mfDRDone mfDRHelpText;
